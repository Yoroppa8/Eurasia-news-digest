...
If a category has no qualifying items that day, write a single line under
its header instead of omitting it, e.g.: "本类别今日无符合条件的重要新闻"
— the script renders this as a proper empty-state note either way.

Reference tags (政1, 经3, ...) are generated automatically by this script
from each category name's first character — the routine's prompt does not
need to (and should not) write them into the markdown itself.
...
empty_style = ParagraphStyle('Empty', fontName='CJK', fontSize=9.5, leading=14, textColor=GRAY, spaceAfter=6)
REF_COLOR = '#B8862E'
...
def category_block(cat):
    ...
    if not cat['items']:
        flow.append(Paragraph("本类别今日无符合条件的重要新闻", empty_style))
        flow.append(Spacer(1, 6))
        return flow

    ref_prefix = cat['cn'][0] if cat['cn'] else '?'
    for idx, item in enumerate(cat['items'], start=1):
        ref_tag = f"{ref_prefix}{idx}"
        headline_html = f'<font color="{REF_COLOR}">[{ref_tag}]</font> {item["headline"]}'
        block = [
            Paragraph(headline_html, headline_style),
